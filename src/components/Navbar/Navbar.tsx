      ...
      >
        <div className="container mx-auto px-4 lg:px-8 max-w-7xl flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              ...
              >
                <a
                  href={link.href}
                  className="flex items-center gap-1 text-sm font-semibold text-text-primary hover:text-green-400 transition-colors py-2"
                >
                  {link.label}
                    ...
                    <div className="glass-card p-2 rounded-xl flex flex-col shadow-card">
                      {link.dropdown.map((item) => (
                        <a
                          key={item.label}
                          href={item.href}
                          className="px-4 py-2.5 text-sm rounded-lg hover:bg-white/5 hover:text-green-400 transition-colors"
                        >
                          {item.label}
                        </a>
                      ))}
          ...
          <div className="hidden lg:flex items-center gap-6">
            <a href="tel:6312031000" className="group relative flex items-center gap-2.5 px-4 py-2 rounded-xl bg-white/5 border border-white/10 overflow-hidden transition-all duration-300 hover:border-green-500/50 hover:bg-green-500/10">
              ...
              <div className="relative z-10 flex flex-col leading-none">
                <span className="text-[8px] uppercase tracking-widest text-green-500/60 font-semibold">Call Now</span>
          ...
          <div className="flex flex-col gap-6 text-2xl font-display font-semibold flex-1">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="border-b border-white/10 pb-4 hover:text-green-400 transition-colors"
