...
export async function POST(req: Request) {
  ...
  // ✅ Acknowledge receipt to Stripe IMMEDIATELY after signature verification.
  // All heavy processing (DB writes, emails, Zapier) runs asynchronously so we
  // never risk hitting Stripe's 30-second response timeout. This was the root
  // cause of the 109 timed-out webhook deliveries starting April 23, 2026.
...
async function handleEvent(event: any) {
  try {
    ...
    console.error(`Unhandled error in webhook event handler for ${event.type}:`, err);
  }
}
