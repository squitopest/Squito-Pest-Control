...
export default function Stats() {
  return (
    <section className="py-24 border-y border-border bg-gradient-to-r from-background via-card to-background overflow-hidden relative" id="stats">
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-5xl relative z-10">
        <div className="hidden md:grid md:grid-cols-3 gap-8 divide-x divide-border mb-16">
          {stats.map((s, i) => (
            <div key={i} className="flex flex-col items-center text-center px-4">
              ...
              <div className="text-4xl lg:text-6xl font-display font-bold text-foreground mb-2 tracking-tight">
                  <CountUp
                    target={s.num}
                    suffix={s.suffix}
              ...
              <div className="text-sm font-semibold text-green-600 uppercase tracking-wider">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 md:hidden gap-12 gap-y-12 mb-16">
          {stats.map((s, i) => (
            <div key={i} className="flex flex-col items-center text-center">
              ...
              <div className="text-xs font-semibold text-green-600 uppercase tracking-wide leading-tight">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center w-full">
           <Link href="/contact" className="group relative inline-flex items-center gap-4 sm:gap-6 px-8 sm:px-12 py-5 sm:py-6 rounded-full bg-tint-5 backdrop-blur-xl border border-tint-10 overflow-hidden cursor-pointer transition-all duration-500 hover:border-green-500/50 hover:bg-tint-10 hover:shadow-[0_0_40px_rgba(34,197,94,0.2)]">
              {/* Shimmer sweep effect */}
              <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-[rgb(var(--surface-tint)/0.10)] to-transparent group-hover:animate-[shimmer_2s_infinite]" />
              ...
              <div className="relative z-10 text-xl sm:text-3xl font-display font-bold text-foreground tracking-wide">
